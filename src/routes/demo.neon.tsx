							...
							{todos.map((todo: { id: number; title: string }) => (
								...
								>
									<div className="flex items-center justify-between">
										<span className="text-lg font-medium group-hover:text-white/90">{todo.title}</span>
			...
			<div className="bg-black/30 p-6 rounded-lg max-w-xl mx-auto">
				...
				<ul className="space-y-4 text-left list-none">
					<li className="flex items-start">
						...
						<div>
							Use the <code className="bg-black/30 px-2 py-1 rounded">db/init.sql</code> file to create the database
						</div>
					</li>
					<li className="flex items-start">
						<span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-amber-500 text-black font-bold mr-3 min-w-8 min-h-8">
						...
						<div>
							Set the <code className="bg-black/30 px-2 py-1 rounded">DATABASE_URL</code> environment variable to the
