...
import { editWineMetadata, deleteWine, loadWinesForEdit } from '../actions';
...
/**
 * The wine list is no longer server-rendered into this page. It is fetched
 * after the password is entered, so an unauthenticated visit costs zero D1
 * reads. Search re-runs the same gated action instead of navigating.
 */
export default function EditClient() {
  ...
  const load = useCallback(
    async (term: string) => {
      ...
      const result = await loadWinesForEdit(formData);

      if (!result.success) {
        setErrorMsg(result.message || 'Failed to load wines');
        setStatus('error');
        setLoaded(false);
        return;
      }
  ...
  return (
    <div className={styles.adminPage} style={{ maxWidth: '800px' }}>
      <h1>Edit Metadata</h1>
      <p>Quickly fix mislabeled countries and varietals. Enter the password to load the list.</p>
      ...
      >
        <label>Admin Password (Required to load and to save)</label>
        <div style={{ display: 'flex', gap: '1rem' }}>
          ...
          >
            {status === 'loading' ? 'Loading...' : loaded ? 'Reload' : 'Unlock'}
          </button>
        </div>
      </form>

      {status === 'error' && (
        <p style={{ color: 'red', fontSize: '0.9rem', marginBottom: '2rem' }}>{errorMsg}</p>
      )}
...
function EditForm({
  ...
  const handleSave = async () => {
    ...
    // Append the precision trailing zeroes for SQLite ISO sorting consistency
